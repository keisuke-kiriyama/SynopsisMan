...
from util.text_processor import get_BoW_vectors, cos_sim, wakati
from util.paths import SIMILARITY_BETWEEEN_CONTENTS_AND_SYNOPSIS_SENTENCE_DIR_PATH

corpus_accessor = CorpusAccessor()
rouge = Rouge()

def most_similarity_between_contents_and_synopsis_sentence_cossim(ncode):
    """
...
def most_similarity_between_contents_and_synopsis_sentence(ncode):
    ...
    for line_idx, contents_line in enumerate(wakati_contents_lines):
        ...
        scores = [rouge.get_scores(hyps=contents_line, refs=synopsis_line, avg=False)[0]['rouge-1']['r'] for synopsis_line in wakati_synopsis_lines]
