...
contents_origin_file_paths = [os.path.join(paths.ORIGIN_CONTENTS_DATA_DIR_PATH, file_name)
...
def execute():
    ...
    スクレイピングしたデータをdata/origin/contentsにまとめ、この関数を回す
    前処理されたファイルはdata/preprocessed/contents下に保存される
    :return:
    """
    for i, contents_origin_file_path in enumerate(contents_origin_file_paths):
        ncode = accessor.ncode_from_file_path(contents_origin_file_path)
        if i % 50 == 0:
            print('progress: {:.1f}%, processing: {}'.format(i / len(contents_origin_file_paths) * 100, ncode))
        origin_data = accessor.load(file_path=contents_origin_file_path)
        data = dict()
        ...
        output_file_path = accessor.create_contents_file_path(ncode=ncode)
